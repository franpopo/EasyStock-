...
def filtrar_lista(event):
    texto = EntryBusqueda.get().lower()
    ListaProductos.delete(0, tk.END)
    for p in productos:
        detalle = f"{p['nombre']} | stock: {p['stock']} | precio: {p['precio']}"
        if texto in detalle.lower():
            ListaProductos.insert(tk.END, detalle)

EntryBusqueda.bind("<KeyRelease>", filtrar_lista)
    ...
    try:
        ...
        if not all(col in columnas for col in ["nombre", "stock", "precio"]):
            messagebox.showerror(
                "Error",
                "El archivo debe tener las columnas: nombre, stock, precio.\n"
        ...
        for _, fila in df.iterrows():
            ...
            codigo_barras = str(fila["codigo_barras"]) if "codigo_barras" in df.columns else None

            cursor.execute(
                "INSERT INTO productos (nombre, stock, precio, codigo_barras) VALUES (?, ?, ?, ?)",
        ...
        messagebox.showerror("Error", "No se encontró el archivo productos.xlsx en la carpeta del programa.")
    ...
    def aceptar():
        ...
        if ModificandoProducto:
            ...
            try:
                ...
                ConexionSql.commit()
                nuevo["id"] = cursor.lastrowid
                productos.append(nuevo)
                ListaProductos.insert(tk.END,
                                      f"{nuevo['nombre']} | stock: {nuevo['stock']} | precio: {nuevo['precio']}")
...
ListaProductos: Listbox = tk.Listbox(ventana, width=40, height=13, font=("Arial", 15), bg="grey")
ListaProductos.grid(row=1, column=0, columnspan=2, sticky="nsew", padx=5, pady=5)

CargarProductos()

BotonAgregar = tk.Button(ventana, text="Agregar producto", bg="lightgreen",
                         command=lambda: MenuAgregarProducto(False))
BotonAgregar.grid(row=2, column=0, columnspan=2, sticky="nsew", padx=5, pady=1)

BotonCargarExcel = tk.Button(ventana, text="Cargar producto por excel", bg="yellow", command= CargarDesdeExcel)
BotonCargarExcel.grid(row=3, column=0, columnspan=2, sticky="nsew", padx=5, pady=1)

BotonModificar = tk.Button(ventana, text="Modificar producto", bg="sky blue", command=CambiarProducto)
BotonModificar.grid(row=4, column=0, columnspan=2, sticky="nsew", padx=5, pady=1)

BotonBorrar = tk.Button(ventana, text="Eliminar producto", bg="tomato", command=EliminarProducto)
BotonBorrar.grid(row=5, column=0, columnspan=2, sticky="nsew", padx=5, pady=1)

BotonVenta = tk.Button(ventana, text="Registrar Venta", bg="orange", command=MenuRegistrarVenta)
BotonVenta.grid(row=6, column=0, columnspan=2, sticky="nsew", padx=5, pady=1)
